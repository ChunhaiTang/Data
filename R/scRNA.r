...
scRNA_RNA = subset(scRNA, subset=nFeature_RNA>200&nFeature_RNA<10000&percent.mito<10) 
...
cell_label = c("T cell","Mesothelial cell","Macrophage","Fibroblast","Plasma Cell","MKI67+ progenitor cel","Endothelial cell","B cell")
names(cell_label) <- levels(mydata)
mydata <- RenameIdents(mydata, cell_label)
mydata[["cell_type"]] = Idents(mydata)
mydata = subset(mydata, cell_type %in% c("T cell","Mesothelial cell","Macrophage","Fibroblast","Plasma Cell","MKI67+ progenitor cel","Endothelial cell","B cell"))
###############################################################
custome_theme_1=theme(axis.text.y=element_text(family="Times",face="plain")
                      ,axis.text.x=element_text(family="Times",face="plain")
                      ,plot.title = element_text(hjust = 0.5,family="Times",face="plain")
...
genes = c("CD2","NKG7","CD7","KRT18","KRT8","KRT19","C1QA","C1QB","LYZ","LUM",
          "DCN","CD79A","IGHG1","IGHG3","MKI67","TOP2A","VWF","PECAM1","IGFBP7","CD79B","MS4A1")
