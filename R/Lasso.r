...
group.v <- c(rep("case",354),rep('control',88)) %>% factor(.,levels = c("case","control"),ordered = F)
...
plot(fit_stage.v,xlab='Log Lambda',cex.lab = 1)+
  text(x = log(fit_stage.v$lambda.min),y = 0.1,
       paste('Lambda.min\n',round(fit_stage.v$lambda.min,4)),cex=0.8,adj=0.1)+
  text(x = log(fit_stage.v$lambda.1se),y = 0.2,
...
plot(fit.v, xvar="lambda",cex.lab = 1)+
  ...
  text(x = log(fit_stage.v$lambda.min),y = 0.1,
       paste('Lambda.min\n',round(fit_stage.v$lambda.min,4)),cex=0.8,adj=0.1)+
  text(x = log(fit_stage.v$lambda.1se),y = 0.2,
       paste('Lambda.lse\n',round(fit_stage.v$lambda.1se,4)),cex=0.8,adj=0.1)
dev.off()
